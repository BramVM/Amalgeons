...
func request_dir(dir_vec: Vector2) -> void:
	var destiny_cell = Grid.to_cell(_body.global_position+dir_vec*GameGlobals.TILE_SIZE)
	if moving:
...
func _start_step(dir_vec: Vector2) -> void:
	current_dir = dir_vec
	_from = _body.position
	_to = _from + dir_vec * GameGlobals.TILE_SIZE
	from_cell = Grid.to_cell(_body.global_position)
...
func physics_tick(delta: float) -> void:
	...
	if _percent >= 1.0:
		...
		if _queued_dir != Vector2.ZERO:
			...
			var destiny_cell = Grid.to_cell(_body.global_position+next*GameGlobals.TILE_SIZE)
